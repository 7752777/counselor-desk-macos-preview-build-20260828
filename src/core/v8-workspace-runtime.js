  ...
  function createWorkspace(options) {
    ...
    function mutate(command) {
      return enqueue(async () => {
        const nextCommand = await cloneCooperatively(command, cooperativeScheduler);
        if (root && typeof root.CWBRequireWorkspaceMutation === 'function') root.CWBRequireWorkspaceMutation(nextCommand);
        return runMutation(nextCommand);
      });
    }
    ...
    function hydrateBackup(backup, actor) {
      return enqueue(async () => {
        ...
        if (root && typeof root.CWBRequireWorkspaceMutation === 'function') root.CWBRequireWorkspaceMutation({ collection:'workspace', operation:'hydrate_backup', actor:actor || 'local-user', data:candidateState });
    ...
    function restoreRecoveryPoint(id, actor) {
      ...
      return enqueue(async () => {
        ...
        if (root && typeof root.CWBRequireWorkspaceMutation === 'function') root.CWBRequireWorkspaceMutation({ collection:'workspace', operation:'restore', actor:actor || 'local-user', data:candidateState });
