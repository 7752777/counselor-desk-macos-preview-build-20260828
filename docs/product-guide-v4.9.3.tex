...
\hypersetup{
  ...
  urlcolor=CWBblue,
  citecolor=CWBblue,
  pdftitle={学工智伴 v4.9.3 前瞻版完整功能产品手册},
  pdfauthor={学工智伴维护组},
  bookmarksopen=true
}
...
\graphicspath{{../assets/screenshots/v4.9.0/}{../assets/screenshots/v4.9.2/}}
...
这是一份给实际使用者准备的操作手册。当前对外产品名称为“学工智伴”，历史截图或旧构建物中的“一站式辅导员工作台”属于兼容名称。内容按照辅导员的日常工作顺序编排，先说明模块适合解决什么问题，再说明常用按钮、查看结果和后续跟进方式。第一次使用时，建议先用示例数据走一遍流程；未激活时只能浏览、筛选和查看样例，不能新增、编辑、删除、导入、恢复或同步真实资料，也不能改写样例。完成基础版或更高档位激活后，再从桌面端录入自己的工作内容。
...
范围 & 说明 \\
\midrule
适用版本 & v4.9.3 前瞻版，适合前瞻体验用户先用示例数据熟悉完整工作流程。 \\
...
首次打开建议先使用虚构样例数据浏览。样例学生、任务、谈话、成绩、住宿、课表、班团、资料、科研和通知带有演示标记，只用于理解按钮和练习流程，不应当直接当作真实事实导出或上报。未激活时只能查看样例和样例快照；真实资料新增、编辑、删除、导入、备份恢复以及真实文件上传，必须先激活基础版或更高档位。
...
\begin{workflowbox}
需要录入自己的资料时，点击右上角盾牌图标，或在“设置 -> 授权、购买与版权”中打开授权窗口。窗口会先提示“授权服务已就绪”；输入前瞻兑换码后会先确认授权服务连接，再显示“正在核验”；成功时会弹出“激活完成”，并列出本端已解锁的真实数据、文件上传、AI 与更新权益。若没有出现“激活完成”，不要反复提交或发送激活码截图；先检查当前交付包版本、网络和窗口显示的错误提示。
\end{workflowbox}
...
“AI 对话”页用于普通问题、工作思路整理和草稿起草。默认不读取学生档案、谈话、附件或其他业务记录；只有老师主动勾选“附带当前范围的脱敏业务上下文”，选择学生或班级，并先查看发送范围后，系统才会发送相应的脱敏记录。对话内容只保留在当前页面会话，刷新或关闭后自动清除，不进入业务备份、导出或局域网同步包。

AI 的回答不会自动改写学生事实、成绩、住宿、考勤或业务记录。需要采纳时，请回到原业务页面人工确认。已获得友情托管 AI 资格的前瞻体验用户不需要填写 API Key、模型名称或服务地址；页面只显示托管服务状态、上游波动说明和重试入口。

\uiShot{ai-chat.png}{AI 对话：默认不附带学生数据，发送前预览范围并保留人工确认边界}{fig:ai-chat}
...
照片、证书、活动图片、查课查寝材料、困难认定材料和科研文件进入附件仓。业务记录只保存附件 ID、文件名、大小、哈希和引用关系。附件支持缩略图、预览、下载、删除、索引导出和重复识别。附件仓是 Windows/macOS 桌面端的长期工作区能力：文件原样保存到工作台数据目录的加密附件仓，换机时与 SQLite、索引和备份一起带走；网页端不建立个人附件仓。
...
运行方式 & 主要存储 & 使用边界 \\
\midrule
浏览器 & 临时本地队列、浏览器快照 & 适合查询和临时录入；关闭期间不能后台同步，不作为个人附件仓或长期备份位置。 \\
唯一离线 HTML & 浏览器本地工作区 & 电脑和手机浏览器共用同一个文件；需单独激活，和桌面端同步数据但不传递授权；新版本需要重新获取离线文件。 \\
Electron & SQLite、WAL、加密附件仓、系统安全存储 & 正式长期工作区；适合导入、文件归档、数据目录、恢复口令、定时备份和局域网主机。 \\
...
桌面端在工作台运行期间按定时器检查备份条件，通常按每日一次或变更量阈值生成加密备份。桌面备份包含 SQLite、附件索引和附件仓，适合换机和长期保存。浏览器端只保留当前页面会话所需的临时状态和离线队列，不应替代桌面端备份；浏览器关闭后无法后台同步，页面必须明确显示这一限制。
...
推荐的长期工作方式是“办公室桌面端为主，下班后唯一离线网页版继续，手机端做移动补录”。局域网主机由 Electron 主进程提供 SQLite、加密附件仓和同步服务；电脑浏览器、手机浏览器和桌面端不是三份互相覆盖的数据库，网页端和手机端通过 HTTPS、证书指纹和一次性配对码或二维码接入主机。

同步操作包含工作区、设备、操作 ID、幂等键、集合、记录、基础修订号和字段补丁。不同字段修改可以自动合并；同字段冲突保留双方值、设备、时间和来源，进入冲突收件箱，人工选择主机值、客户端值或手动编辑。

附件二进制不放入业务同步记录。网页端和手机端只同步经过授权的业务字段与离线队列，不承担个人附件上传和长期保存；需要表格、模板、政策 PDF、照片或业务附件时，请先在桌面端归档。设备可以暂停、恢复和撤销，配对、同步、冲突和附件操作写入本地最小审计。

\section{三端互通的实际流程}

\begin{enumerate}
  \item 在办公室 Electron 桌面端启动局域网数据中枢，确认 HTTPS 地址和证书指纹；桌面端作为正式工作区主机。
  \item 在主机生成一次性配对码或二维码，核对有效期和工作区标识。
  \item 手机或网页端输入/扫描配对信息，人工核对地址和指纹后向主机发起请求；首次配对只需完成一次。
  \item 主机确认设备，客户端取得一次性设备令牌并连接。
  \item 下班后保持已配对的网页端页面继续工作，修改先保存在本地离线队列；第二天回到办公室、主机运行且页面回到前台时，客户端先提交离线队列，再拉取远程修订，也可点击“立即同步”。如果网页已关闭，重新打开后需重新连接或配对。
  \item 遇到同字段冲突时人工选择，解决结果形成新的修订，其他设备再次拉取。
  \item 主机定期备份工作区；换机时使用恢复包恢复主机，再重新配对客户端。
\end{enumerate}

主机离线或暂时不在同一网络时，客户端可以保留加密离线队列；浏览器关闭期间不会继续后台同步，重新打开并完成连接后再重试。局域网首期只面向同一网络，不等同于公网协作平台，也不自动信任二维码中的证书。冲突不会静默覆盖，必须在冲突收件箱人工确认。
...
激活、更新、AI、备份和局域网同步等联网操作都会生成一条可追踪的请求链。链路尽量沿用同一个请求 ID，按“客户端发起 -> 网关接收 -> 应用处理 -> 上游返回 -> 客户端完成”的顺序记录。遇到问题时，先记录页面显示的版本、时间、操作名称和错误码，再用请求 ID定位，不需要重复提交激活码。

\begin{table}[H]
  \centering
  \caption{联网请求的可核对节点}
  \label{tab:network-diagnostics}
  \small
  \begin{tabularx}{\textwidth}{B{29mm} P{52mm} Y}
    \toprule
    节点 & 记录位置 & 可以确认什么 \\
    \midrule
    网页渲染层 & 当前浏览器的诊断环形日志 & 是否发起、是否发送、状态码、耗时和错误码 \\
    Electron 主进程 & \makecell[l]{用户数据目录的\\\code{logs/}\\\code{network-diagnostics.jsonl}} & 系统网络栈是否收到响应、请求和响应字节数 \\
    公网网关 & Nginx 脱敏访问日志 & DNS/TLS/反向代理是否到达以及上游状态 \\
    授权服务 & 授权服务网络日志 & 许可证、更新权限和 CORS 业务结果 \\
    AI Relay 与上游 & Relay 网络日志 & 托管 AI 或模型上游是否握手、返回和超时 \\
    局域网主机 & 桌面端诊断日志 & 配对、同步队列和附件分块请求结果 \\
    \bottomrule
  \end{tabularx}
\end{table}

每条诊断记录只保留时间、请求 ID、规范化操作名、传输方式、脱敏路径、阶段、状态码、耗时、字节数和脱敏错误码。不会记录激活码、API Key、Authorization、请求正文、完整响应、学生资料或附件二进制。阶段通常是 \code{started -> request\_sent -> response -> completed}；失败或取消会在对应阶段明确结束。

\begin{enumerate}
  \item 先在同一台设备打开授权服务健康地址，确认 DNS、TLS 和公网入口可达。
  \item 回到工作台重新执行一次需要排查的操作，记录页面中的请求 ID和错误码。
  \item 从设置中的诊断入口导出诊断包；桌面端同时保留用户数据目录中的 JSONL 日志。
  \item 按请求 ID依次检查客户端、Nginx、授权服务或 Relay，区分“请求未到达”和“服务已返回业务错误”。
  \item 反馈时只提供版本、时间、页面、错误码、请求 ID和脱敏截图，不发送激活码、API Key或真实业务附件。
\end{enumerate}

如果客户端有发送记录而服务端没有同一请求 ID，优先检查 DNS、TLS、反向代理和公网穿透；如果服务端有请求并返回 4xx，优先检查许可证、更新权益或 CORS 规则；如果返回 5xx，再检查数据库、签名器、文件存储、Relay 或上游服务。诊断日志用于定位链路，不代表系统会自动上传业务数据。
...
浏览器适合快速查询和临时工作；唯一离线 HTML 适合手机或下班后的临时录入；Electron 适合长期使用、附件管理、数据目录、定时备份和局域网主机。涉及学生照片、证书、政策 PDF、大量资料或换机迁移时，应使用 Electron 桌面端并建立可恢复备份。
...
AI 按钮不能使用 & 先检查 AI 授权、模型连接、用途授权、敏感字段授权、模型类型和额度；文本可用不代表语音端点可用。 \\
激活后没有完成提示 & 确认使用本轮网络诊断交付包，检查是否显示“授权服务已就绪”和明确错误提示；可从诊断入口导出请求 ID、状态码和错误码，不要把激活码发到聊天、公众号或截图中。 \\
